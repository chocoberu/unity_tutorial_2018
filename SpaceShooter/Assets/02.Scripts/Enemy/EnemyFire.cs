...
public class EnemyFire : MonoBehaviour
{
    ...
    private Transform playerTr; // 주인공 캐릭터의 Transform 컴포넌트
    private Transform enemyTr; // 적 캐릭터의 Transform 컴포넌트
    ...
    void Update()
    {
        if(isFire && !isReload)
        {
            ...
            Quaternion rot = Quaternion.LookRotation(playerTr.position - enemyTr.position); // 주인공이 있는 위치까지의 회전 각도 계산
    ...
    void Fire()
    {
        ...
        StartCoroutine(ShowMuzzleFlash()); // 총구 화염 효과 코루틴 호출

        GameObject _bullet = Instantiate(Bullet, firePos.position, firePos.rotation); // 총알을 생성
        Destroy(_bullet, 3.0f); // 일정 시간이 지난 후 삭제 
    ...
    IEnumerator ShowMuzzleFlash() 
    {
        ...
        Quaternion rot = Quaternion.Euler(Vector3.forward * Random.Range(0, 360)); // 불규칙한 회전 각도를 계산
        muzzleFlash.transform.localRotation = rot; // MuzzleFlash를 Z축 방향으로 회전
        muzzleFlash.transform.localScale = Vector3.one * Random.Range(1.0f, 2.0f); // MuzzleFlash의 스케일을 불규칙하게 조정

        Vector2 offset = new Vector2(Random.Range(0, 2), Random.Range(0, 2)) * 0.5f; // 텍스처의 Offset 속성에 적용할 불규칙한 값을 생성
        muzzleFlash.material.SetTextureOffset("_MainTex", offset); // MuzzleFlash의 머티리얼의 Offset 값을 적용

        yield return new WaitForSeconds(Random.Range(0.05f, 0.2f)); // MuzzleFlash가 보일 동안 잠시 대기
        muzzleFlash.enabled = false; // MuzzleFlash를 다시 비활성화
    }
    IEnumerator Reloading()
    {
        ...
        currBullet = maxBullet; // 총알 개수를 초기화
        isReload = false;
    }
}
